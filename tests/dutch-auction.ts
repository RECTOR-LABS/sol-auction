...
describe("dutch_auction", () => {
  ...
  it("buys Dutch auction at current price", async () => {
    ...
    const buyerAta = await createAssociatedTokenAccount(
      provider.connection, payer, mint, buyer.publicKey,
    );
  ...
  it("rejects buy after auction already settled", async () => {
    ...
    await program.methods
      .createAuction(auctionId, { dutch: { startPrice: new anchor.BN(5e9), reservePrice: new anchor.BN(1e9) } }, new anchor.BN(now - 60), new anchor.BN(now + 3540))
      .accounts({ seller: provider.wallet.publicKey, auctionHouse: housePda, itemMint: mint, sellerItemAccount: sellerAta })
      .rpc();

    // First buyer
    const buyer1 = anchor.web3.Keypair.generate();
    await provider.connection.confirmTransaction(await provider.connection.requestAirdrop(buyer1.publicKey, 20e9));
    const buyer1Ata = await createAssociatedTokenAccount(provider.connection, payer, mint, buyer1.publicKey);
    ...
    await provider.connection.confirmTransaction(await provider.connection.requestAirdrop(buyer2.publicKey, 20e9));
    const buyer2Ata = await createAssociatedTokenAccount(provider.connection, payer, mint, buyer2.publicKey);
